...
public class RMF_RadialMenu : MonoBehaviour {
    ...
    public RectTransform offset_rt;
    ...
    void Update() {
        ...
        if (!useGamepad){
            if(offset_rt == null)
                ...
                rawAngle = Mathf.Atan2(Input.mousePosition.y - offset_rt.position.y, Input.mousePosition.x - offset_rt.position.x) * Mathf.Rad2Deg;
        }
        else
        ...
        if(offset_rt != null && fakeCursor != null) {
            fakeCursor.position = rt.position + new Vector3(Input.mousePosition.x - offset_rt.position.x, Input.mousePosition.y - offset_rt.position.y, 0);
        }
