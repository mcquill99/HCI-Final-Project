...
namespace VHS
{
    ...
    public class FirstPersonController : MonoBehaviour
    {
        #region Variables
            #region Public      
                ...
                #region Locomotion
                    ...
                    [BoxGroup("Locomotion Settings")] public float slideSpeed = 1f;
                    [BoxGroup("Locomotion Settings")] public float slopedSlideSpeed = 1f;
                ...
                #region Slide Settings
                    [Space]
                    [BoxGroup("Slide Settings")] public float slideDuration = 2.5f;
                    [BoxGroup("Slide Settings")] public float slopeMinAngle = 15f;
                    [BoxGroup("Slide Settings")] public bool requireSprintToSlide = true;
                    [BoxGroup("Slide Settings")][MinMaxSlider(0f,90f)]  public Vector2 steepSlopeAngles = new Vector2(40f, 60f);
                    [BoxGroup("Slide Settings")] public AnimationCurve slideSpeedCurve = AnimationCurve.EaseInOut(0f,0f,1f,1f);
                #endregion
            ...
            #region Private
                ...
                #region Debug/Cache
                    ...
                    [BoxGroup("DEBUG")][SerializeField][ReadOnly] float m_hasJumpedCooldown;
                    ...
                    [BoxGroup("DEBUG")][SerializeField][ReadOnly] float m_slideProgress;
                    [BoxGroup("DEBUG")][SerializeField][ReadOnly] float m_slopedSlideTimestamp;
                    [BoxGroup("DEBUG")][SerializeField][ReadOnly] float m_slideTimestamp;
                    [BoxGroup("DEBUG")][SerializeField][ReadOnly] Vector3 m_slideDir;
                    ...
                    [BoxGroup("DEBUG")][SerializeField][ReadOnly] int m_currentJumpCount;
        ...
        #region BuiltIn Methods     
            ...
            void Update()
            {
                Debug.DrawLine(transform.position, transform.position + m_finalMoveVector * 10f, Color.green);
        ...
        #region Custom Methods
            ...
            #region Locomotion Calculation Methods
                ...
                void CalculateMovementDirection()
                {
                    ...
                    //Movement is not completely input based when sliding
                    if(movementInputData.IsSliding) {
                        _desiredDir = (m_slideDir + (1 - slideSpeedCurve.Evaluate(m_slideProgress)) * _desiredDir).normalized;
                    }
            ...
            #region Crouching Methods
                void HandleCrouch()
                {
                    if(m_slopedSlideTimestamp > Time.time) {
                        return;
                    }
                    if(movementInputData.IsCrouching && movementInputData.RunClicked) {
                        movementInputData.IsSliding = false;
                        m_cameraController.isLockedCamera = false;
                        InvokeCrouchRoutine();
                    }

                    if(movementInputData.IsCrouching && !movementInputData.CrouchHeld && m_slopedSlideTimestamp < Time.time) {
                        InvokeCrouchRoutine();
                    }
                ...
                void InvokeCrouchRoutine(bool bypassAnim = false)
                {
                    if(movementInputData.IsCrouching) {
                        if(CheckIfRoof() || m_slopedSlideTimestamp > Time.time)
                            return;
                        movementInputData.IsSliding = false;
                        m_cameraController.isLockedCamera = false;
                    } else {
                        //if moving and moving forward
                        Vector3 rotatedMoveDir = m_finalMoveVector.normalized;
                        Vector2 flattenedMoveDir = new Vector2(rotatedMoveDir.x, rotatedMoveDir.z);
                        Vector2 flattenedRightDir = new Vector2(transform.right.x, transform.right.z);

                        //check for initiating slide
                    ...
                    if(!bypassAnim) {
                        ...
                        //Does CrouchRoutine() but without the easing
                        ...
                        _camPos.y = movementInputData.IsCrouching ? m_initCamHeight : m_crouchCamHeight;
                        m_characterController.height = _desiredHeight;
                        m_characterController.center = _desiredCenter;
                    }
                ...
                void HandleSlide() {
                    ...
                    Vector3 _origin = transform.position + m_characterController.center;
                    bool _hitGround = Physics.Raycast(_origin,Vector3.down,out m_hitInfo,m_finalRayLength + raySphereRadius,groundLayer);
                    ...
                    if(_hitGround && m_hasJumpedCooldown < Time.time) {
                        ...
                        if(angle > steepSlopeAngles.x && angle < steepSlopeAngles.y) {
                            ...
                            m_cameraController.isLockedCamera = true;
                            m_slideTimestamp = Time.time + slideDuration;
                            m_slopedSlideTimestamp = Time.time + 0.1f;

                            
                            Vector3 normal = m_hitInfo.normal;
                            normal.y = 0;
                            m_slideDir = normal.normalized;
                        } else {
                            //print("NOT FORCED SLIDING");
                        }
                    }

                    if(movementInputData.IsSliding) {
                        _origin = transform.position + m_characterController.center;
                        _hitGround = Physics.Raycast(_origin,Vector3.down,out m_hitInfo,m_finalRayLength + raySphereRadius,groundLayer);

                        //if on an angled slope, maintain speed and adjust slide direction
                        if(_hitGround) {
                            if(Vector3.Angle(m_hitInfo.normal, Vector3.up) > slopeMinAngle) {
                                m_slideTimestamp = Mathf.Clamp(m_slideTimestamp + Time.deltaTime * 2f, 0, Time.time + slideDuration);
                                Vector3 normal = m_hitInfo.normal;
                                normal.y = 0;
                                m_slideDir = Vector3.Lerp(m_slideDir, normal.normalized, Time.deltaTime * 0.8f);
                            }
                        }
            ...
            #region Locomotion Apply Methods
                ...
                void HandleJump()
                {
                    ...
                    if(movementInputData.JumpClicked && movementInputData.IsCrouching) {
                        ...
                        m_currentJumpCount = Mathf.Clamp(m_currentJumpCount + 1, 0, jumpAmount);
                ...
                void ApplyGravity()
                {
                    ...
                    else
                    {
                        ...
                        //Multi-jump
                        if(m_currentJumpCount < jumpAmount - 1) {
                            HandleJump();
                        }
                    }
                }
                ...
                void RotateTowardsCamera()
                {
                    ...
                    //Locks camera movement to slide direction
                    if(movementInputData.IsSliding) {
                        Vector3 flattenedDir = m_slideDir;
                        flattenedDir.y = 0;
                        _desiredRot = Quaternion.LookRotation(flattenedDir.normalized, Vector3.up);
                        transform.rotation = Quaternion.Slerp(_currentRot,_desiredRot,Time.deltaTime * 6f);
